...
either serve structural purposes, by adding in bifurcation ot the structure, or contain states equipped with transition and emission probabilites.
...
end we have develped an algorithm that utilizes the Zhang Shasha Tree Edit distance algorithm, augmented with node to node comparison that uses a HMM Distance comparison 
funcion. This algorithm takes as input two CM's and measures their structureal similarity. 

We validate our algorithm as finding the smallest edit distance between two \cm s in Section~\ref{sec:formalism}. To validate the efficacy of our approach we 
compare our results to the \clique{} of RNA families. These \clique s encode how similar families are to each-other based upon empirical evidence. 
% \br{verify the name and purpose of clicks} 
By comparing the distance between models, to their respective \clique{} we infer a cutoff and validate upon a hold out
set. We evaluate this approach in Section~\ref{sec:evaluation}.
